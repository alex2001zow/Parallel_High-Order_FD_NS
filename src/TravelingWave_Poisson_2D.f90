module TravelingWave_Poisson_2D_module
   ...
   use FD_module, only: FDstencil_type, create_finite_difference_stencils, deallocate_finite_difference_stencil, &
      calculate_scaled_coefficients, get_FD_coefficients_from_index, &
   ...
   integer, parameter :: max_iter = 1, multigrid_max_level = 1
...
contains
   ...
   subroutine TravelingWave_Poisson_2D_main(rank, world_size)
      ...
      call calculate_velocity_divergence(u_params, v_params, FDstencil_params, p_params)
   ...
   recursive subroutine w_cycle(solver, comm, p_block_fine, FDstencil, result, max_level, level)
      ...
      ! Pre-smoothing
      call solve_pressure_poisson(comm, p_block_fine, FDstencil, &
         p_block_fine%block_begin_c, p_block_fine%f_matrix_ptr_2D, solver, result)
      ...
      if(level /= max_level) then
         ...
         call sendrecv_data_neighbors(comm%comm, p_block_coarse, p_block_coarse%matrix_ptr)
         call sendrecv_data_neighbors(comm%comm, p_block_coarse, p_block_coarse%f_matrix_ptr) ! Not sure if this is needed
   ...
   subroutine Write_analytical_function(u_block, v_block, p_block)
      ...
      do ii = u_block%block_begin_c(2)+1, u_block%block_end_c(2)
         do jj = u_block%block_begin_c(1)+1, u_block%block_end_c(1)
            ...
            p_block%f_matrix_ptr_2D(p_local_indices(1),p_local_indices(2)) = uu_x + ww_z
   ...
   subroutine calculate_velocity_divergence(u_block, v_block, FDstencil, p_block)
      ...
      do ii = u_block%block_begin_c(2)+1, u_block%block_end_c(2)
         do jj = u_block%block_begin_c(1)+1, u_block%block_end_c(1)
            ...
            call get_FD_coefficients_from_index(u_block%ndims, FDstencil%num_derivatives, FDstencil%stencil_sizes, &
               [1,1], u_block%extended_block_dims, uv_local_indices, FDstencil%scaled_stencil_coefficients, alpha, beta, &
               coefficients)
   ...
   subroutine solve_pressure_poisson(comm, p_block, FDstencil, rhs_block_begin_c, rhs_matrix_2D, solver, result)
      type(comm_type), intent(in) :: comm
      type(block_type), intent(inout) :: p_block
      type(FDstencil_type), intent(inout) :: FDstencil
      integer, dimension(:), intent(in) :: rhs_block_begin_c
      real, dimension(:,:), intent(inout) :: rhs_matrix_2D
      type(SolverParamsType), intent(in) :: solver
      type(ResultType), intent(out) :: result
      ...
      do while(converged /= 1 .and. it < solver%max_iter)
         call poisson_iteration(p_block%block_begin_c, p_block%block_end_c, p_block%extended_block_dims, &
            rhs_block_begin_c, p_block%matrix_ptr, p_block%matrix_ptr_2D, rhs_matrix_2D, FDstencil, norm_array(1))
   ...
   subroutine poisson_iteration(p_block_begin_c, p_block_end_c, p_extended_block_dims, rhs_block_begin_c, &
      p_matrix, p_matrix_2D, rhs_matrix_2D, FDstencil, local_norm)
      integer, dimension(:), intent(in) :: p_block_begin_c, p_block_end_c, p_extended_block_dims, rhs_block_begin_c
      real, dimension(:), intent(inout) :: p_matrix
      real, dimension(:,:), intent(inout) :: p_matrix_2D, rhs_matrix_2D
      type(FDstencil_type), intent(inout) :: FDstencil
      real, intent(out) :: local_norm

      integer :: color, ii, jj
      integer, dimension(ndims) :: p_local_indices, rhs_local_indices, alpha, beta
      real, contiguous, dimension(:), pointer :: coefficients, dxx, dss
      real, dimension(product(FDstencil%stencil_sizes)), target :: combined_stencils
      real, contiguous, dimension(:,:), pointer :: combined_stencils_2D
      ...
      do color = 0, 1
         ...
         !$omp shared(p_block_begin_c, p_block_end_c, p_extended_block_dims, rhs_block_begin_c, p_matrix_2D, &
         !$omp rhs_matrix_2D, FDstencil, color) &
         !$omp private(ii, jj, p_local_indices, rhs_local_indices, alpha, beta, coefficients, dxx, dss, combined_stencils, &
         !$omp combined_stencils_2D, old_val, f_val, new_val)
         do ii = p_block_begin_c(2)+1, p_block_end_c(2)
            do jj = p_block_begin_c(1)+1, p_block_end_c(1)
               if(mod(ii+jj,2) /= color) cycle ! Avoid the if-statement somehow...
               p_local_indices = [jj,ii]
               rhs_local_indices = rhs_block_begin_c + p_local_indices - p_block_begin_c

               old_val = p_matrix_2D(jj,ii)
               f_val = rhs_matrix_2D(rhs_local_indices(1), rhs_local_indices(2))

               call get_FD_coefficients_from_index(ndims, FDstencil%num_derivatives, FDstencil%stencil_sizes, &
                  [1,1], p_extended_block_dims, p_local_indices, &
                  FDstencil%scaled_stencil_coefficients, alpha, beta, coefficients)
               ...
               call update_value_from_stencil_2D(combined_stencils_2D, p_matrix_2D, p_local_indices, alpha, beta, &
                  -f_val, new_val)

               new_val = (1.0 - omega) * old_val + omega * new_val

               p_matrix_2D(jj,ii) = new_val
