module Poisson_module
   ...
   use utility_functions_module, only: open_txt_file, close_txt_file, IDX_XD, IDX_XD_INV, sleeper_function, swap_pointers, &
      reshape_real_1D_to_2D, print_real_2D_array, calculate_dt_from_CFL
   ...
   ! Time parameters
   real, parameter :: t_begin = 0.0, t_end = 10.0
   real :: current_time = t_begin
   real :: t_steps, dt

   !> Solver parameters
   integer :: solver_type = 0
   real, parameter :: tol = (1e-12)**2, div_tol = 1e-1, omega = 1.0
   integer, parameter :: max_iter = 100 * (1.0 + 1.0 - omega), multigrid_max_level = 1
...
contains
   ...
   subroutine Poisson_main(rank, world_size)
      ...
      call create_block_type(ndims, 1, 1, domain_begin, domain_end, grid_size, comm_params, &
         ghost_begin, ghost_end, stencil_begin, stencil_end, solver_type, block_params)

      call create_finite_difference_stencils(ndims, num_derivatives, derivatives, stencil_sizes, FDstencil_params)

      !call calculate_dt_from_CFL(1.0, [1.0,1.0], block_params%grid_dx, dt)
      !t_steps = (t_end - t_begin) / dt + 1
      t_steps = 1
      dt = (t_end - t_begin) / t_steps
      ...
      if(solver_type == 1) then
         ...
         call write_inital_condition(block_params)
   ...
   subroutine one_timestep(block_params, FDstencil_params, comm_params, result)
      ...
      if(solver_type == 1) then
         ...
         block_params%temp_matrix_ptr_2D = block_params%temp_matrix_ptr_2D + (dt/-2.0) * block_params%matrix_ptr_2D
         ...
         block_params%matrix_ptr_2D = block_params%matrix_ptr_2D + (dt/-2.0) * block_params%f_matrix_ptr_2D
      end if
   ...
   recursive subroutine w_cycle(solver, comm, block_fine, FDstencil, result, max_level, level)
      ...
      if(level == max_level) then
         call GS_Method(comm, block_fine, FDstencil, solver_params, result)
         ...
         ! Residual errors is done inside the GS_Method
   ...
   !> Write the initial condition to the system
   subroutine write_inital_condition(block_params)
      type(block_type), intent(inout) :: block_params
      ...
      do ii = block_params%extended_block_begin_c(2)+1, block_params%extended_block_end_c(2)
         do jj = block_params%extended_block_begin_c(1)+1, block_params%extended_block_end_c(1)
            ...
            call u_analytical_poisson(block_params%ndims, point, t_begin, u_val)

            block_params%temp_matrix_ptr_2D(jj,ii) = u_val
   ...
   subroutine GS_iteration_2D(block_params, FDstencil, local_u_diff_norm, local_r_diff_norm, local_u_norm, local_r_norm)
      ...
      do ii = block_params%block_begin_c(2)+2, block_params%block_end_c(2)-1
         do jj = block_params%block_begin_c(1)+2, block_params%block_end_c(1)-1
            ...
            block_params%residual_matrix_ptr_2D(jj,ii) = r1_val
   ...
   subroutine calculate_rhs_2D(block_params)
      ...
      do ii = block_params%extended_block_begin_c(2)+1, block_params%extended_block_end_c(2)
         do jj = block_params%extended_block_begin_c(1)+1, block_params%extended_block_end_c(1)
            ...
            call f_analytical_poisson(ndims, point, current_time, f_val)
   ...
   !> Write the Dirichlet boundary conditions to the rhs
