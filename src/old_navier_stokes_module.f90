...
contains
   ...
   subroutine Navier_Stokes_2D_main(rank, world_size)
      ...
      call set_SolverParamsType(Pressure_Poisson_tol, 1e-1, N_Pressure_Poisson_iterations, 2, solver_params)
   ...
   subroutine Navier_Stokes_2D_analytical_wrapper(ndims, rank, world_size, grid_size, processor_dims, domain_begin, domain_end, &
      ...
      call create_block_type(ndims, 1, 1, domain_begin, domain_end, grid_size, comm_params, &
         stencil_sizes*0,stencil_sizes*0, stencil_sizes*0, stencil_sizes*0, stencil_sizes/2, stencil_sizes/2, u_block_params)

      call create_block_type(ndims, 1, 1, domain_begin, domain_end, grid_size, comm_params, &
         stencil_sizes*0,stencil_sizes*0, stencil_sizes*0, stencil_sizes*0, stencil_sizes/2, stencil_sizes/2, v_block_params)

      call create_block_type(ndims, 1, 1, domain_begin, domain_end, grid_size, comm_params, &
         stencil_sizes*0,stencil_sizes*0, stencil_sizes*0, stencil_sizes*0, stencil_sizes/2, stencil_sizes/2, p_block_params)
   ...
   subroutine NS_2D_one_timestep(comm, u_block, v_block, p_block, FDstencil, solver_params_in, rho, nu, dt, converged, it)
      ...
      call write_velocity_BC(u_block%ndims, u_block%extended_block_dims, u_block%matrix, v_block%matrix)
